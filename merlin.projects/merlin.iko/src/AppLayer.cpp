...
void AppLayer::InitGraphics() {
	...
	helix->translate(glm::vec3(60, 0, 140));
	helix->scale(0.05);
	helix->hide();
	helix->setMaterial("cyan plastic");
	//helix->rotate(glm::vec3(0, -20*DEG_TO_RAD, 0));

	half_helix = ModelLoader::loadMesh("./assets/models/HELIX_HALF.stl");
	half_helix->rotate(glm::vec3(0, 0, 180 * DEG_TO_RAD));
	half_helix->translate(glm::vec3(60, 0, 140));
	half_helix->scale(0.05);
	half_helix->setMaterial("cyan plastic");
	//helix->rotate(glm::vec3(0, -20*DEG_TO_RAD, 0));

	domain = ModelLoader::loadMesh("./assets/models/DOMAIN.stl");
	domain->rotate(glm::vec3(0, 0, 180 * DEG_TO_RAD));
	domain->translate(glm::vec3(60, 0, 140));
	domain->scale(0.05);
	domain->setMaterial("white plastic");
	domain->hide();
	
	half_domain = ModelLoader::loadMesh("./assets/models/DOMAIN_HALF.stl");
	half_domain->rotate(glm::vec3(0, 0, 180 * DEG_TO_RAD));
	half_domain->translate(glm::vec3(60, 0, 140));
	half_domain->scale(0.05);
	half_domain->setMaterial("white plastic");

	//scene.add(emitter);
	scene.add(domain);
	scene.add(half_domain);
	scene.add(helix);
	scene.add(half_helix);
...
void AppLayer::Simulate(Merlin::Timestep ts) {
	...
	half_helix->rotate(glm::vec3(0, -30 * DEG_TO_RAD, 0));
	half_helix->rotate(glm::vec3(-rot_speed * settings.timestep.value(), 0, 0));
	half_helix->rotate(glm::vec3(0, 30 * DEG_TO_RAD, 0));
...
void AppLayer::onImGuiRender() {
	...
	static bool showhdomain = true;
	if (ImGui::Checkbox("Show half domain", &showhdomain)) {
		if (showhdomain) half_domain->show();
		else half_domain->hide();
	}

	static bool showdomain = false;
	if (ImGui::Checkbox("Show domain", &showdomain)) {
		if (showdomain) domain->show();
		else domain->hide();
	}

	static bool showhelix = true;
	if (ImGui::Checkbox("Show helix", &showhelix)) {
		if (showhelix) helix->show();
		else helix->hide();
	}
