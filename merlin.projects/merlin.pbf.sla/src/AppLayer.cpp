...
void AppLayer::InitGraphics() {
	...
	emitter = Primitives::createCube(200, 110, 20);
	emitter->enableWireFrameMode();
	emitter->translate(glm::vec3(0,0,10));

	vat = ModelLoader::loadMesh("./assets/models/vat.stl");
	vat->rotate(glm::vec3(DEG_TO_RAD * 90, 0, 0));
	vat->setMaterial("black plastic");
	 
	bed = ModelLoader::loadMesh("./assets/models/bed.stl");
	bed->translate(glm::vec3(0, 0, 5));
...
void AppLayer::ResetSimulation() {
	...
	{
		vat->computeBoundingBox();
		vat->voxelize(spacing);
		BoundingBox aabb = vat->getBoundingBox();
		glm::vec3 bb_size = aabb.max - aabb.min;
		int gridSizeX = ceil(bb_size.x / spacing);
		int gridSizeY = ceil(bb_size.y / spacing);
		int gridSizeZ = ceil(bb_size.z / spacing);
