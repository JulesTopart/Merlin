...
void AppLayer::InitGraphics() {
	...
	bed = ModelLoader::loadMesh("./assets/models/bed.stl");
	bed->rotate(glm::vec3(DEG_TO_RAD * 90, 0, 0));
	bed->computeBoundingBox();
	bed->translate(glm::vec3(0, -bed->getBoundingBox().min.z - 50, 0));
	...
	dirlight->translate(dirlight->direction() * glm::vec3(-1000));
	...
	scene.add(createShared<AmbientLight>("light4", glm::vec3(0.8)));
}
...
void AppLayer::ResetSimulation() {
...

	{
		...
		for (int i = 0; i < gridSizeX * gridSizeY * gridSizeZ; i++) {
			if (emitter->getVoxels()[i] != 0) {
				...
				cpu_temperature.push_back(298.15);
	...
	{
		spacing = settings.particleRadius * 1.5;
		bed->computeBoundingBox();
		bed->voxelize(spacing);
		BoundingBox aabb = bed->getBoundingBox();
		glm::vec3 bb_size = aabb.max - aabb.min;
		int gridSizeX = ceil(bb_size.x / spacing);
		int gridSizeY = ceil(bb_size.y / spacing);
		int gridSizeZ = ceil(bb_size.z / spacing);
	...
	{
		pistonA->computeBoundingBox();
		pistonA->voxelize(spacing);
		BoundingBox aabb = pistonA->getBoundingBox();
		glm::vec3 bb_size = aabb.max - aabb.min;
		int gridSizeX = ceil(bb_size.x / spacing);
		int gridSizeY = ceil(bb_size.y / spacing);
		int gridSizeZ = ceil(bb_size.z / spacing);
	...
	{
		pistonB->computeBoundingBox();
		pistonB->voxelize(spacing);
		BoundingBox aabb = pistonB->getBoundingBox();
		glm::vec3 bb_size = aabb.max - aabb.min;
		int gridSizeX = ceil(bb_size.x / spacing);
		int gridSizeY = ceil(bb_size.y / spacing);
		int gridSizeZ = ceil(bb_size.z / spacing);
		...
		for (int i = 0; i < gridSizeX * gridSizeY * gridSizeZ; i++) {
			if (pistonB->getVoxels()[i] != 0) {
				...
				cpu_velocity.push_back(glm::vec4(0));
				cpu_temperature.push_back(298.15);
				cpu_meta.push_back(glm::uvec4(BOUNDARY + 2, numParticles, numParticles, 0.0));
				numParticles++;
			}
		}
	}
	...
	{
		roller->computeBoundingBox();
		roller->voxelize(spacing);
		BoundingBox aabb = roller->getBoundingBox();
		glm::vec3 bb_size = aabb.max - aabb.min;
		int gridSizeX = ceil(bb_size.x / spacing);
		int gridSizeY = ceil(bb_size.y / spacing);
		int gridSizeZ = ceil(bb_size.z / spacing);
		...
		for (int i = 0; i < gridSizeX * gridSizeY * gridSizeZ; i++) {
			if (roller->getVoxels()[i] != 0) {
				...
				cpu_velocity.push_back(glm::vec4(0));
				cpu_temperature.push_back(298.15);
				cpu_meta.push_back(glm::uvec4(BOUNDARY + 3, numParticles, numParticles, 0.0));
				numParticles++;
			}
		}
	}
...
void AppLayer::Simulate(Merlin::Timestep ts) {
	...
	if (step == 0) {
		pistonAPos += settings.solver_substep * 0.02;
