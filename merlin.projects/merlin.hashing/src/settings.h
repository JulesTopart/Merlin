...
struct Bin {
	...
	GLuint hash; //bin index
};

struct Settings {
	const float particleRadius = 0.2;
	...
	glm::vec3 bb = glm::vec3(150, 60, 130);//update this in the shader as well


	// Physics Parameters
	Uniform<float> timestep							= Uniform<float>("dt", 0.0016*0.25);
