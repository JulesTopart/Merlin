...
void AppLayer::InitPhysics() {
	...
	ps->addField<float>("PressureBuffer");
	ps->addField<float>("cpyPressureBuffer");
	...
	ps->link(particleShader->name(), "PressureBuffer");
...
void AppLayer::ResetSimulation() {
	...
	auto cpu_pressure = std::vector<float>();
	...
	ps->writeField("PressureBuffer", cpu_pressure.data());
...
void AppLayer::Simulate(Merlin::Timestep ts) {
	float xPos = 20 * (cos(elapsedTime * 5.0) + 1);
	solver->use();
	solver->setFloat("xPos", xPos);
	dam->setPosition(glm::vec3(-5 + xPos -settings.bb.x / 2.0, -settings.bb.y / 2, 0));
