...
void AppLayer::InitGraphics() {
	...
	solidA = Primitives::createCube(80,80,4);
	solidB = Primitives::createSphere(10, 30, 30);
	solidB->translate(glm::vec3(0, 0, 15));
...
void AppLayer::InitPhysics() {
	...
	ps->addField<glm::vec4>("rest_position_buffer");
	...
	ps->link(particleShader->name(), "rest_position_buffer");
	...
	ps->link(texPlot->name(), "rest_position_buffer");
...
void AppLayer::ResetSimulation() {
	...
	ps->writeField("rest_position_buffer", cpu_position);
