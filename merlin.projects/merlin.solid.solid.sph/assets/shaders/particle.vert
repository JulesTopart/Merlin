...
void main() {
	...
	if(colorCycle == 0){ 
		...
		float r = map(abs(ssbo_sigma[i].xx), 0, maxS);
		...
		if(gl_InstanceID == testsortedID){
			...
			binTest = false;
			uvec3 binIndexVec3 = getBinCoord(ssbo_rest_position[testsortedID].xyz);
			...
			vec3 position = ssbo_rest_position[testsortedID].xyz;
			OVERNNS
				if(gl_InstanceID == j){
					nnTest = true;
					if(distance(ssbo_rest_position[testsortedID].xyz,  ssbo_rest_position[j].xyz) < cst_smoothingRadius) hTest = true;
				}
			OVERNNS_END
