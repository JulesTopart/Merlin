...
void AppLayer::InitGraphics() {
	...
	static_emitter = Primitives::createCylinder(1, 1, 10);
	static_emitter->enableWireFrameMode();
	static_emitter->translate(glm::vec3(0, 0, 20));
...
void AppLayer::ResetSimulation() {
...

	{
		...
		floor->voxelizeSurface(spacing, spacing);
		BoundingBox aabb = floor->getBoundingBox();
		glm::vec3 bb_size = aabb.max - aabb.min;
		int gridSizeX = ceil(bb_size.x / spacing);
		int gridSizeY = ceil(bb_size.y / spacing);
		int gridSizeZ = ceil(bb_size.z / spacing);
...
void AppLayer::Simulate(Merlin::Timestep ts) {
	...
	float dt = glm::min<float>(ts.getSeconds(), settings.timestep.value());

	elapsedTime += dt;

	float s_dt = dt /settings.solver_substep;

	solver->use();
	solver->setFloat("dt", s_dt);
	...
	GPU_PROFILE(solver_substep_time,
		for (int i = 0; i < settings.solver_substep; i++) {
			if (integrate) {
				time += s_dt;
				if (time - lastSpawTime > spawnDelay) {
					SpawnParticle();
					lastSpawTime = time;
				}
...
void AppLayer::onImGuiRender() {
	...
	ImGui::SliderFloat("Emitter delay", &spawnDelay, 0.0, 0.016);
