...
void main() {
	...
	float temp = length(_normal);
	if(temp < 275.5 + 25.0) temp = 275.5 + 25.0;
	vec4 color = colorMap(map(temp,275,275+300.0), inferno);
	vout.color = vec3(color);
	...
	if(length(_tangent) == 0 || length(_bitangent) == 0 || length(_normal) == 0){
		...
		vec3 N = normalize(vec3(model * vec4(normalize(_normal), 0.0)));
