...
void AppLayer::InitPhysics() {
	...
	ps->addField<glm::dvec4>("last_position_buffer");
	ps->addField<glm::dvec4>("position_buffer");
	ps->addField<glm::dvec4>("predicted_position_buffer");
	ps->addField<glm::dvec4>("correction_buffer");
	ps->addField<glm::dvec4>("velocity_buffer");
	ps->addField<double>("density_buffer");
	ps->addField<double>("lambda_buffer");
	ps->addField<double[8]>("stress_buffer");
	ps->addField<double[12]>("rotation_buffer");
...
void AppLayer::ResetSimulation() {
	...
	auto cpu_position = std::vector<glm::dvec4>();
	...
	for (int i = 0; i < positions.size(); i++) {
		cpu_position.push_back(glm::dvec4(positions[i],0));
...
void AppLayer::Simulate(Merlin::Timestep ts) {
	...
	if (pullDistance < 20) {
		...
		solver->setDouble("u_pullDistance", pullDistance);
	}
...
void AppLayer::onImGuiRender() {
	...
	if (ImGui::Button("Debug")) {
		...
		ps->getField("density_buffer")->readBuffer(settings.pThread*sizeof(float), metabuf.data());
