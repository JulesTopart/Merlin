...
void AppLayer::SyncUniforms() {
	...
	settings.laser_transform = glm::translate(settings.laser_transform(), glm::vec3(-20, 0, 50));
...
void AppLayer::InitGraphics() {
	...
	powder = Primitives::createCylinder(10, 30, 20);
	powder->translate(glm::vec3(0, 0, 10));
...
void AppLayer::ResetSimulation() {
	...
	for (int i = 0; i < positions.size(); i++) {
		...
		if(positions[i].z <= 1 && false)cpu_meta.push_back(glm::uvec4(SOLID, settings.numParticles(), settings.numParticles(), 0.0));
