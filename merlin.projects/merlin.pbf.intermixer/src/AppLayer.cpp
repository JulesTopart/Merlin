...
void AppLayer::SyncUniforms() {
	...
	settings.emitter_transform = glm::translate(settings.emitter_transform(), glm::vec3(0, 0, 0));
...
void AppLayer::InitGraphics() {
	...
	bbox->translate(glm::vec3(0,0, settings.bb.z*0.5));
	scene.add(bbox);
	...
	scene.add(capA);
	scene.add(capB);
	scene.add(static_emitterA);
	scene.add(static_emitterB);
...
void AppLayer::ResetSimulation() {
	...
	static_emitterA->computeBoundingBox();
	static_emitterA->voxelize(spacing);
	positions = Voxelizer::getVoxelposition(static_emitterA->getVoxels(), static_emitterA->getBoundingBox(), spacing);
	...
	positions = Voxelizer::getVoxelposition(static_emitterB->getVoxels(), static_emitterB->getBoundingBox(), spacing);
	...
	for (int i = 0; i < positions.size(); i++) {
		cpu_position.push_back(glm::vec4(positions[i], 0));
		cpu_temp.push_back(275.15 + 60); //ambient
	...
	spacing = settings.particleRadius * 1.0;
	capA->computeBoundingBox();
	capA->voxelize(spacing);
	positions = Voxelizer::getVoxelposition(capA->getVoxels(), capA->getBoundingBox(), spacing);
	...
	spacing = settings.particleRadius * 1.0;
	capB->computeBoundingBox();
	capB->voxelize(spacing);
	positions = Voxelizer::getVoxelposition(capB->getVoxels(), capB->getBoundingBox(), spacing);
