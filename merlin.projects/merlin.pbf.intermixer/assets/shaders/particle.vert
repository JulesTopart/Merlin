...
void main() {
	...
	bool test = phase(gl_InstanceID) == UNUSED || ((phase(gl_InstanceID) == BOUNDARY || phase(gl_InstanceID) == 4) && showBoundary == 0);
	color = vec4(1);
	if(colorCycle == 0){ 
		...
		if(phase(gl_InstanceID) == FLUIDA) color = vec4(0.2,1.0,0.2, 1);
		else if(phase(gl_InstanceID) == FLUIDB) color = vec4(0.2,0.6,1.0, 1);
	}else{ //NNS Test
