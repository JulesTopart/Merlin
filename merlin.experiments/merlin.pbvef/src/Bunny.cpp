...
std::vector<glm::vec3> GenerateVoxelBunny(float voxel_size) {
	std::vector<glm::vec3> data;
	for(int x = 0; x < 32; x++)
	for(int y = 0; y < 32; y++)
	for(int z = 0; z < 32; z++) {
		if ( sampleBunny(glm::vec3(x / 32.0, y / 32.0, z / 32.0)) > 0.0) {
			data.push_back(vec3(x * voxel_size - 16, y * voxel_size - 16, z * voxel_size) + vec3(0.5));
		}
	}
	return data;
}