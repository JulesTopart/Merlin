...
void ExampleLayer::OnAttach(){
	...
	renderer_ptr = CreateScope<Renderer>();
	scene_ptr = CreateScope<Scene>();
	...
	Shared<Model> model55 = Model::Create("sphere24", GetModel());
	...
	model55->SetMaterial("aluminum");
	model55->SetShader("pbr");
...
void ExampleLayer::OnUpdate(Timestep ts){
	...
	renderer_ptr->Clear();
	renderer_ptr->RenderScene(*scene_ptr, *camera);
}
