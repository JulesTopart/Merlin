...
void main() {
	...
	if (hasNormalTex == 1)	gNormal = normalize(texture(normal0, texCoord).rgb);
	else					gNormal = normalize(normal);

	if (hasColorTex == 1)	gAlbedoSpec.rgb = texture(color0, texCoord).rgb;
	else					gAlbedoSpec.rgb = color.rgb;

	if (hasSpecularTex == 1)gAlbedoSpec.a = length(texture(specular0, texCoord).rgb);
	else					gAlbedoSpec.a = length(specular.rgb);
