...
vec3 triIntersect( in vec3 ro, in vec3 rd, in vec3 v0, in vec3 v1, in vec3 v2 )
{
    ...
    if( u<0.0 || v<0.0 || (u+v)>1.0 ) t = -1.0; //No intersection ? What if the coordinate is x -1.0 when colliding (not so probable)
    return vec3( t, u, v );
}
...
void main() {
  ...
  for(int i = 0; i < size; i++){   //Loop for identify the facet intersected by the ray
	Facet f = facets [i];
	...
	//Intersection test, result : Facet id
		...
		vec3 d = vertices[f.indices[3]].position;
		
		vec3 pt1 = quadIntersect( r.o, r.d, a, b, c, d);

		if(pt1 != vec3(-1.0)){ //the good facet
  ...
  if(r.hitID == -1){//No intersection with the geometry, Ray goes to the sky or vaccum
	...
	if(Ptfloor == vec3(-1.0)){//no intersection, the ray hit the sky
		//r.hitID = ;
