...
vec4 pointLight(){	
	...
	vec3 specularColor;
	if (hasSpecularTex == 1) specularColor = specular * texture(specular0, uv).rgb;
	...
	return vec4(ambientColor + (diffuseColor * (diffuseAmount * inten) + specularAmount * specularColor * inten) * vec3(lightColor), 1.0);
}
...
vec4 direcLight()
{
	...
	vec3 specularColor;
	if (hasSpecularTex == 1) specularColor = texture(specular0, uv).rgb;
	else specularColor = color;
